...
Author: Jesik Min
Overview: Baseline CNN with mse
    ...
    def on_epoch_end(self, batch, logs={}):
        ...
        if cur_val_loss < self.best_val_loss:
            ...
            _write_1D_deeplift_track(predictions.reshape(X_train.shape[0], window_size),
                                     normalized_train_intervals, os.path.join(srv_dir, 'train'))
            _write_1D_deeplift_track(val_predictions.reshape(X_val.shape[0], window_size),
                                     normalized_val_intervals, os.path.join(srv_dir, 'val'))
